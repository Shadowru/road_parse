...
class MetaParser {
  ...
  parse(line) {
    ...
    _jsLogger2.default.debug('After region: ' + region_data.line);

    const roads_data = this._cutRoads(region_data.line);

    const division_data = this._cutDivision(region_data.line);

    _jsLogger2.default.debug('After division: ' + division_data.line);

    const places_data = this._cutPlace2(division_data.line); //const places_data2 = this._cutPlace(places_data.line);
    //mutated_line = this._cutPlace(mutated_line);


    _jsLogger2.default.debug('Regions: ' + region_data.regions);

    _jsLogger2.default.debug('Divisions: ' + division_data.divisions);

    _jsLogger2.default.debug('Places: ' + places_data.places); //Logger.debug('Places2: ' + places_data2.places);


    _jsLogger2.default.debug('Line: ' + places_data.line);
  }
  ...
  _cutPlace2(line) {
    ...
    for (const type_element of type_dict) {
      ...
      while (true) {
        ...
        const index_pos = mutated_line.indexOf(prefix, pos);

        if (index_pos !== -1) {
          pos = index_pos;

          const pos_cut = this._simpleFindByType(mutated_line, pos + prefix.length, type, this._delta);
          ...
          if (pos_cut !== undefined) {
            ...
            places.push(pos_cut[0].type + ' ' + data.cut);
  ...
  _simpleFindByType(line, pos, type, delta) {
    for (const dict_element of this._dict.dict) {
      if (this._isInList(dict_element.type, type)) {
        ...
        const pos_spc = this._findInLine(line, start_pos, name);
  ...
  _cutLine(line, pos_cut) {
    ...
    if (pos_cut.length !== 0) {
      ...
      let delta = 0; //Logger.debug(pos_cut);

      for (const cutElement of pos_cut) {
        const start_cut = cutElement.start - delta;
        const end_cut = cutElement.end - delta; //Logger.debug(cutElement.start + "/" + cutElement.end + " | " + start_cut + "/" + end_cut);

        const cut_element = mutated_line.substring(start_cut, end_cut);
        data.push(cut_element);
        mutated_line = this.cut(mutated_line, start_cut, end_cut);
